...
export function AdminHomeSchedule({
  ...
  const [selectedStaffId, setSelectedStaffId] = useState<string>(
    staff.length > 1 && !hideFilters ? ALL_STAFF_ID : (staff[0]?.id || ALL_STAFF_ID),
  );
            ...
            <div className="flex flex-wrap items-center gap-2">
              {staff.length > 1 ? (
                <Button
                  radius="full"
                  size="sm"
                  className={`h-8 border px-4 text-[11px] font-bold uppercase tracking-wider transition-colors ${
                    selectedStaffId === ALL_STAFF_ID
                      ? 'border-transparent bg-slate-100 text-slate-900'
                      : 'border-white/10 bg-transparent text-slate-400 hover:bg-white/5 hover:text-slate-300'
                  }`}
              ...
              {staff.map((item) => (
                <Button
                  ...
                  radius="full"
                  size="sm"
                  className={`h-8 border px-4 text-[11px] font-medium tracking-wide transition-colors ${
