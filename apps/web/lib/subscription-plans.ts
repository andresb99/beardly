export type SubscriptionTier = 'free' | 'pro' | 'business' | 'app_admin';
...
export const SUBSCRIPTION_PLAN_CATALOG: Record<SubscriptionTier, SubscriptionPlanDescriptor> = {
  ...
  pro: {
    ...
    features: ['Todo Free + recordatorios', 'Cursos + convocatorias de modelos', 'Metricas operativas del negocio'],
  ...
  business: {
    ...
    features: ['Todo Pro + multi-sede', 'Roles avanzados y permisos granulares', 'Soporte prioritario y onboarding'],
    canBypassPayments: false,
  },
  app_admin: {
    id: 'app_admin',
    ...
    monthlyPriceCents: 0,
    annualInstallmentCents: 0,
    features: ['Acceso total de plataforma', 'Switch de planes para pruebas', 'Sin bloqueo de pagos para QA'],
    canBypassPayments: true,
  },
};
...
export function getSubscriptionPriceCents(tier: SubscriptionTier, billingMode: SubscriptionBillingMode) {
